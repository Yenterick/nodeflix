...
const registerUser = async (req, res) => {
    try {
        ...
        log(await userModel.selectUserByEmail(email.trim()));
        if (await userModel.selectUserByEmail(email.trim())) return res.status(400).json({ success: false, msg: 'Email already exists.' });
